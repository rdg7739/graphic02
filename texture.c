/*
 * @Brief
 *    create a marble texture for fun
 *         ... shamelessly ripped from glut example
 * @Jian Chen (jichen@umbc.edu)
 *
 * [VIEWING]:  No need to change this file
 */
...
extern GLubyte terrainTexture[TXSX][TXSY][3];
...
void GenerateTextures(void)
{
	...
	for (j=0;j<TXSY;j++)
	{
	    ...
	    terrainTexture[i][j][0] = (0.6*t)*255;
	    terrainTexture[i][j][1] = (0.4*t)*255;
	    terrainTexture[i][j][2] = (0.5-0.4*t)*255;
	}
    }
}
